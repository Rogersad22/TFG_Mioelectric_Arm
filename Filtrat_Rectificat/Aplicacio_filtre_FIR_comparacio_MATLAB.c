...
double Coeficients_FIR[251] = {-3.819836317e-19, 4.155086572e-06, 0.0001888578554, 0.0003796207893, 0.0003908814397,
...
float calculMAV(double mostres[48000])
{
        double suma = 0.0;
...
float calculRMS(double mostres2[48000])
{
        float sumaquadrats = 0.0;
        float promig = 0.0;
        float rms = 0.0;

        for (int i = 0; i < 48000; i++)
        {
                sumaquadrats += mostres2[i] * mostres2[i];
        }
        promig = sumaquadrats / 48000;
...
void convolucio(double senyal[48000])
{
        ...
        float valor_rms = calculRMS(senyal_EMG_filtrada);
        printf("El resultat del RMS del senyal filtrat és: %.2f\n", valor_rms);
        float valor_mav = calculMAV(senyal_EMG_filtrada);
        printf("El resultat del MAV del senyal filtrat és: %.2f\n", valor_mav);
        printf("OK9\n");
        return;
}
...
void main(void)
{
        ...
        double senyal_EMG[48000];

        printf("OK1");

        FILE *file;

        file = fopen("Dades_Senyal_Captura14_Roger_Exterior.txt", "r");
        ...
        for (int i = 0; i < 48000; i++)
        {
                if (fscanf(file, "%d,%lf", &contador, &senyal_EMG[i]) != 2)
                {
                        printf("ERROR");
                        return;
                }
