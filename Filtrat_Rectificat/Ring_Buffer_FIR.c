...
double ProcessatFIR(CircularBuffer *cb, double senyal_EMG)
{
        double FIR = 0.0;
        double FIR_rec = 0.0;
...
int main(void)
{
        ...
        file = fopen("Dades_Senyal_Captura1.txt", "r");
        file_2 = fopen("FE_Programa_C.txt", "w");
        ...
        {
                // if (fscanf(file, "%d,%lf", &contador1, &senyal_EMG[i]) != 2)
                //{
                //         printf("ERROR");
                //         return 1;
                // }
                if (fscanf(file, "%d,%lf,%f", &contador1, &senyal_EMG[i], &dummy) != 3) // Va guardant cada dada del fila en la varaible senyal_EMG[i]
                {
                        printf("ERROR");
                        return 1;
                }
                ...
                {
                        senyal_EMG_filtrada[i] = ProcessatFIR(&cb, senyal_EMG[i]);
                        fprintf(file_2, "%d,%lf\n", i, senyal_EMG_filtrada[i]);
                }
                // usleep(333);
        }
        fclose(file);
