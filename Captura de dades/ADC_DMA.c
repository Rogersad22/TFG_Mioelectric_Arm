    ...
    while (1)
    {
        ...
        for (int i = 0; i < frame.len; i += SOC_ADC_DIGI_RESULT_BYTES)
        {
            ...
            if (count == 20)
            {
                ...
                if (mostres_enviades <= 48000)
                {
                    int32_t mv1 = (int32_t)voltatge_mv1;
                    int32_t mv2 = (int32_t)voltatge_mv2;
                    //printf("Valors de mostres enviades %ld, mv1 %ld, mv2 %ld", mostres_enviades, mv1, mv2);
                    //int32_t mv1 = (int32_t)voltatge_mv1;
                    //int32_t mv2 = (int32_t)voltatge_mv2;
                    paquet[0] = 0xAA;
                    paquet[1] = 0xCD;
                    memcpy(&paquet[2], &mostres_enviades, 4);
                    memcpy(&paquet[6], &mv1, 4);
                    memcpy(&paquet[10], &mv2, 4);
                    usb_serial_jtag_write_bytes(paquet, 14, portMAX_DELAY);
                   
                }
                
                else
                {
                    mostres_enviades = 1; // reseteja directament a 1
                    continue;             // o simplement no enviïs aquest paquet
                }


                /*if (mostres_enviades == 21000)
                // AIXÒ TARDA 5s.
                {
